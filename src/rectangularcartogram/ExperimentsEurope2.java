...
public class ExperimentsEurope2 {
    ...
    private static void loop() throws IOException, IncorrectGraphException, IloException {
        String name = "Europe Electricity Production";
        Subdivision sub = loadSubdivision(new File("../Data/Europe/" + name + ".sub"));
        ...
        while (true) {
            ...
            saveSubdivision(cartogram, new File("Results/" + name, cartogram.getAverageCartographicError() + " " + cartogram.getMaximumCartographicError() + " " + measureNames[(m - 1 + measures.length) % measures.length] + ".sub"));
        }
    }
